...
| 告警 | 规则增删改；`> >= < <=` 阈值 + 警告/严重两级；触发图表变色、横幅与桌面弹窗，回落自动解除；数据源被关闭或转入异常时其激活告警立即解除 |
...
测试通过环境变量 `ENABLE_TEST_API=true` 开启仅测试用的确定性注入/淘汰/故障/节拍接口（`/api/test/*`，生产编排中为 `false`），因此无需等待随机模拟时序即可精确锁定行为。
...
7. **源停用即解除告警**（`tests/alert-source.test.ts`）：手动关闭或采集异常（error）时，该源名下激活告警立即产生 `resolved` 事件、移出激活列表并经 WS 推送；重开后遗留告警不复活、按新读数正常判定；其它健康源的告警不受牵连。
