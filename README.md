...
7. **源关闭即解除**（`tests/source-alerts.test.ts`）：数据源被手动关闭（或进入采集异常）时，其名下所有激活告警立即解除——激活列表清空、事件记录出现 `resolved`、WS 同步推送 `alert_event`；重新打开后旧告警不残留，安全值不误触发、越限照常触发；健康源的告警不受其它源开关牵连。
